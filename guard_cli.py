...
class GuardCLI(SSHClient):
    ...
    def __init__(self, host, port, username, password, timeout=None, missing_host_key_policy=True, sock=None):
        ...
        try:
            ...
            logger.error('Cannot open an SSH session with the Guardium instance.')
            logger.info(str(e))
        ...
        logger.info(self.interact.current_output_clean)
    ...
    def exec_cmd(self, command, print_stdout=True):
        ...
        logger.info(self.username + "@" + self.host + "]# {}".format(command))  # log the command
    ...
    def get_installed_patches(self):
        ...
        try:
            if 'USAGE:' in result:
                logger.error('Unable to get list of installed patches because an installation is underway.')
                return results_list
    ...
    def start_file_server(self, source_ip_address, timeout=3600):
        ...
        while 'is' not in result:
            ...
            if timeout_counter > 30:
                logger.error('A timeout occurred while waiting for the fileserver to start.')
        ...
        if 'The file server is ready' in result:
            ...
            if fileserver_link:
                logger.info(fileserver_link[0])
                return True
            else:
                return True

        elif 'already running' in result:
            logger.info('Fileserver is already running.')

            return True

        else:
            logger.info('A problem occurred while trying to start the fileserver.')
            logger.info(result)
    ...
    def build_file_server_link(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error('Unable to determine IP Address of Guardium instance.')
    ...
    def install_patch(self, patch_number):
        ...
        logger.info('store system patch install sys')
        ...
        if 'No files in the patches directory to install' in result:
            ...
            self.interact.send('\n')
            self.interact.expect('.*')
            logger.info('There are no available patches to install.')
            return 'There are no available patches to install.'

        choice_number = self.get_install_choice_number(result, patch_number)
        logger.info('Choice number for patch ' + patch_number + ' is ' + choice_number)
        ...
        logger.info(self.interact.current_output_clean)  # Install item 3 (or whatever the choice was)
        # Blank line u''
    ...
    def get_install_choice_number(self, result, patch_number):
        ...
        logger.error('Unable to locate an install choice for patch #' + patch_number)
        raise Exception('Unexpected problem occurred when trying to install patch #' + \
                        str(patch_number) + '. Please try again.')
    ...
    def delete_scheduled_patch(self, patch_number):
        ...
        logger.info('delete scheduled-patch')
        self.interact.expect('.*', timeout=30)

        logger.info('--------Patches that can be deleted list:------')
        logger.info(self.interact.current_output_clean)
        logger.info('-----------------------------------------------')
        ...
        logger.info('-------------Remove patch result---------------')
        logger.info(result)
        logger.info('-----------------------------------------------')

        if 'Invalid patch number' in result:

            # Exception occurred.  Quit.
            self.interact.send('q')
            self.interact.expect('.*')
            logger.error(self.interact.current_output_clean)
        ...
        logger.info('Completed delete of scheduled patch: ' + patch_number)
        self.interact.send('\n')
